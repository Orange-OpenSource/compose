...
describe('Get company endpoint', () => {
  ...
  test('Get company details', function(done){
    const mockApiResponse = {
      data: companies.list[0]
    };
  ...
  test('Get company details with company id', function(done){
    const mockApiResponse = {
      data: companies.list[0]
    };
    ...
    api.get(`/api/companies/${companies.list[0].companyId}`)
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(err, res){
        if (err) return done(err);
