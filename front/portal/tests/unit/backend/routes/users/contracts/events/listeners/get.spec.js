...
describe('Get event listener endpoint', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  });
  ...
  test('Do not authorize the access if not authenticated', function(done){
    api.get('/api/users/me/contracts/04be0470-20af-4baa-8726-130bce0b98e8/events/NewStore/webhooks/1234')
  ...
  test('Get event listener', function(done){
    const mockApiResponse = {
      data: eventListeners.list[0]
    };
  ...
  test('Get event listener with user id', function(done){
    const mockApiResponse = {
      data: eventListeners.list[0]
    };
    ...
    api.get('/api/users/f5453f60-2390-48cc-9caa-50c6d143f400/contracts/04be0470-20af-4baa-8726-130bce0b98e8/events/NewStore/webhooks/1234')
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(err, res){
        if (err) return done(err);
