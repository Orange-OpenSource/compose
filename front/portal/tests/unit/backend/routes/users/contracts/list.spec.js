...
describe('List contracts endpoint', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  });
  ...
  test('Do not authorize the access if not authenticated', function(done){
    api.get('/api/users/me/contracts')
  ...
  test('Get contracts list', function(done){
  ...
  test('Get contracts list with user id', function(done){
    const mockApiResponse = {
      data: contracts.list
    };
    ...
    api.get('/api/users/f5453f60-2390-48cc-9caa-50c6d143f400/contracts')
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(err, res){
        if (err) return done(err);
