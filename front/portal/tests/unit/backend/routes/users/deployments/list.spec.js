...
describe('List deployments endpoint', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  });
  ...
  test('Do not authorize the access if not authenticated', function(done){
    api.get('/api/users/me/deployments')
  ...
  test('Get deployments list', function(done){
  ...
  test('Get deployments list with user id', function(done){
    const mockApiResponse = {
      data: deployments.list
    };
    ...
    api.get('/api/users/f5453f60-2390-48cc-9caa-50c6d143f400/deployments')
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(err, res){
        if (err) return done(err);
