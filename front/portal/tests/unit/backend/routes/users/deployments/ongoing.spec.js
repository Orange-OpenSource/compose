...
describe('List ongoing deployments endpoint', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  });
  ...
  test('Do not authorize the access if not authenticated', function(done){
    api.get('/api/users/me/deployments/ongoing')
  ...
  test('Get ongoing deployments list', function(done){
    const mockApiResponse = {
      data: deployments.ongoing
    };
  ...
  test('Get ongoing deployments list with user id', function(done){
    const mockApiResponse = {
      data: deployments.ongoing
    };
    ...
    api.get('/api/users/f5453f60-2390-48cc-9caa-50c6d143f400/deployments/ongoing')
      .expect('Content-Type', /json/)
      .expect(200)
      .end(function(err, res){
        if (err) return done(err);
